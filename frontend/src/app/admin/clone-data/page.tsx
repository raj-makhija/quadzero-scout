...
import { api, CloneJobStatus, CloneOptions } from '@/lib/api';
...
function CloneDataPanel({ stage }: { stage: 'dev' | 'qa' }) {
  ...
  async function handleStart() {
    ...
    setPhase('starting');
    try {
      const { jobId: newJobId } = await api.startCloneProdData(options);
      setJobId(newJobId);
      setPhase('processing');
    } catch (err) {
      ...
      <div className="card p-6 max-w-2xl space-y-6">
        ...
        {phase === 'idle' || phase === 'starting' ? (
          <div className="space-y-3">
            {/* Clone options (mirror the original CLI script flags) */}
            <fieldset className="space-y-2">
              <legend className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                Options
              </legend>
              <CloneOptionToggle
                label="Include resume files (S3)"
                checked={options.includeS3}
                onChange={(v) => setOptions((o) => ({ ...o, includeS3: v }))}
              />
              <CloneOptionToggle
                label="Include config tables (Prompts, Pricing)"
                checked={options.includeConfigTables}
                onChange={(v) => setOptions((o) => ({ ...o, includeConfigTables: v }))}
              />
              <CloneOptionToggle
                label="Clear target before copy (uncheck to merge/incremental)"
                checked={options.clearTarget}
                onChange={(v) => setOptions((o) => ({ ...o, clearTarget: v }))}
              />
              <CloneOptionToggle
                label="Dry run (scan & count only — no changes written)"
