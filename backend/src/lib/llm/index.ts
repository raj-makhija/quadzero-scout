...
You MUST respond with valid JSON matching this exact schema:
{
  ...
  "vendorCompany": "string or null - the recruitment/staffing vendor or sub-vendor company that submitted this candidate, taken from the supplementary email body/signature (NOT the candidate's own employer). Null if not present",
  "vendorContactName": "string or null - the name of the vendor's point of contact from the email signature. Null if not present",
  "vendorContactEmail": "string or null - the vendor contact's email address from the email signature. Null if not present",
  "vendorContactPhone": "string or null - the vendor contact's phone number from the email signature. Null if not present"
}
...
12. For vendorCompany, vendorContactName, vendorContactEmail, vendorContactPhone: these describe the staffing vendor/sub-vendor who submitted this candidate, extracted from the supplementary email body/signature only — NOT from the resume and NOT the candidate's own current employer. Look at the email signature block for a company name, contact person, email, and phone. Set each to null if the supplementary text does not clearly contain it. Never guess`;
