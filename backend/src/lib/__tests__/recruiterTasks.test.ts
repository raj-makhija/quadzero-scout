...
describe('resolveScreeningTasksForCandidate (auto-complete on screen)', () => {
  ...
  it('completes the candidate\'s screen + rescreen pool tasks across requirements', async () => {
    ...
    expect(state.updates.every((u) => u.ExpressionAttributeValues[':c'] === 'completed')).toBe(true);
    expect(state.updates.every((u) => u.ExpressionAttributeValues[':cb'] === 'rec-2')).toBe(true);
  });
