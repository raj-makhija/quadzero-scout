...
import { CLONE_SOURCE_STAGE } from '../../lib/cloneData.js';
...
async function handleRequest(event: AuthenticatedEvent): Promise<APIGatewayProxyResultV2> {
  try {
    ...
    await invokeLambdaAsync(config.lambda.cloneDataWorkerName, { jobId, target });
    ...
    logAuditEvent(event.auth, event, {
      ...
      entityType: 'config',
      entityId: jobId,
      metadata: { jobId, source: CLONE_SOURCE_STAGE, target },
    });
