...
async function handleRequest(
  ...
  try {
    ...
    if (requirementId && cached !== null) {
      // ── Cache read path ───────────────────────────────────────────────────
      // The cache item is present (a completed build). An empty ranked list
      // means the build genuinely matched zero candidates — it flows through
      // this path and naturally resolves to a totalMatches:0 result (#510).
      ...
      // ── Cold-cache pending path ───────────────────────────────────────────
      // Requirement-bound search where the match cache item is absent
      // (getMatchCache → null), i.e. it hasn't been built yet. Running a full
      // live scan on the 30s search Lambda budget times out at prod scale
      // (#510). Instead, (re)dispatch the cache worker and return a lightweight
      // pending response so the client can poll until the cache lands.
      // Note: a present-but-empty cache ([]) is a *completed* zero-match build
      // and is handled by the read path above, not here.
      try {
      ...
      // ── Live-scan path ────────────────────────────────────────────────────
      // Ad-hoc search (no requirementId) only. Runs the full scan + shared
      // scorer, then resolves the page from the in-memory result.
