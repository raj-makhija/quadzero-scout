...
describe('POST /recruiter/search — match-cache read path', () => {
  ...
  // Cold-cache requirement-bound path (#510): returns pending response, dispatches worker.
  it('returns cacheBuilding pending response (not a live scan) when cache is cold (null)', async () => {
    vi.mocked(getMatchCache).mockResolvedValue(null);
  ...
  // Cold-cache pending path: empty ranked list ([]) is also treated as cold (#510).
  it('returns cacheBuilding pending response when cache returns empty ranked list ([])', async () => {
    vi.mocked(getMatchCache).mockResolvedValue([]);
