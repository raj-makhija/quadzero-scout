...
describe('updateRequirementStatus handler', () => {
  ...
  // ticket #469 — reopen (→ active) dispatches the rebuild to the async worker
  // (off the 30s request path); the drop path (→ non-active) deletes the cache
  // inline and never dispatches. The rebuild + #447 failure observability is
  // exercised in matchCacheRequirementWorker.test.ts.
  it('TC-STATUS-469-a: dispatches the cache rebuild to the worker on reopen (→ active)', async () => {
    mockGetRequirementById.mockResolvedValue({ ...activeRequirement, status: 'closed_on_hold' });
  ...
  it('TC-STATUS-469-b: returns 200 with status active even when the reopen dispatch throws', async () => {
    mockGetRequirementById.mockResolvedValue({ ...activeRequirement, status: 'closed_on_hold' });
  ...
  it('TC-STATUS-469-c: deletes the cache and does not dispatch on a non-active transition (drop path)', async () => {
    const result = parseResponse(await handler(makeEvent({ status: 'closed_on_hold' })));
