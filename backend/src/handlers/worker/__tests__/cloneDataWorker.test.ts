...
describe('cloneDataWorker', () => {
  ...
  it('dry run scans but performs no writes, deletes, or S3 copies', async () => {
    await handler({ jobId: 'clone_1', target: 'dev', options: { includeS3: true, includeConfigTables: true, clearTarget: true, dryRun: true } });
    const docCmds = mockDocSend.mock.calls.map((c) => (c[0] as { _cmd: string })._cmd);
    const s3Cmds = mockS3Send.mock.calls.map((c) => (c[0] as { _cmd: string })._cmd);
    expect(docCmds).toContain('scan');
    expect(docCmds).not.toContain('batchwrite');
    expect(s3Cmds).toContain('list');
    expect(s3Cmds).not.toContain('copy');
    expect(s3Cmds).not.toContain('delete');
  });

  it('skips S3 entirely when includeS3 is false', async () => {
    await handler({ jobId: 'clone_1', target: 'dev', options: { includeS3: false, includeConfigTables: true, clearTarget: true, dryRun: false } });
    expect(mockS3Send).not.toHaveBeenCalled();
  });

  it('skips config tables when includeConfigTables is false', async () => {
    await handler({ jobId: 'clone_1', target: 'dev', options: { includeS3: true, includeConfigTables: false, clearTarget: true, dryRun: false } });
    const touched = tableNamesTouched();
    expect(touched.some((n) => n.startsWith('Prompts-'))).toBe(false);
    expect(touched.some((n) => n.startsWith('PricingConfig-'))).toBe(false);
    // Data tables are still cloned.
    expect(touched).toContain('TalentProfiles-prod');
  });

  it('does not clear the target when clearTarget is false', async () => {
    // With clearTarget off, the only scans should hit *-prod (source), never *-dev (clear).
    await handler({ jobId: 'clone_1', target: 'dev', options: { includeS3: false, includeConfigTables: true, clearTarget: false, dryRun: false } });
    const scanTargets = mockDocSend.mock.calls
      .filter((c) => (c[0] as { _cmd: string })._cmd === 'scan')
      .map((c) => (c[0] as { input: { TableName: string } }).input.TableName);
    expect(scanTargets.length).toBeGreaterThan(0);
    expect(scanTargets.every((n) => n.endsWith('-prod'))).toBe(true);
  });
