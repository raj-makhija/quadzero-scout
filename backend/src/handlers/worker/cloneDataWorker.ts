...
import { runCloneJob, normalizeCloneOptions } from '../../lib/cloneData.js';
...
export async function handler(event: CloneDataWorkerEvent): Promise<void> {
  ...
  const options = normalizeCloneOptions(event.options);
  console.log(`Clone data worker started for job ${jobId} → target ${target}`, options);
  ...
  try {
    const result = await runCloneJob(target, options, {
