/**
 * Seed the admin-editable vendor_jd_rewriter prompt for #490.
 * Run with: DYNAMODB_TABLE_PROMPTS=Prompts-<stage> npx tsx scripts/seedVendorJdPrompt.ts
 *
...
import { FALLBACK_VENDOR_JD_REWRITER_PROMPT } from '../src/lib/llm/index.js';

const region = process.env.AWS_REGION || 'ap-south-1';
const tableName = process.env.DYNAMODB_TABLE_PROMPTS || 'Prompts-dev';
const docClient = DynamoDBDocumentClient.from(new DynamoDBClient({ region }));

export const PROMPT_KEY = 'vendor_jd_rewriter';
export const SEED_CONTENT = FALLBACK_VENDOR_JD_REWRITER_PROMPT;
  ...
  await docClient.send(
    new PutCommand({
      ...
      Item: {
        ...
        description: 'Initial seed from codebase (#490)',
      },
    })
  );
...
const isMain = process.argv[1]?.includes('seedVendorJdPrompt');
if (isMain) {
  seed().catch((err) => {
    console.error(err);
    process.exit(1);
