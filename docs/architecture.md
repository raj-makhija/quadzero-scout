...
- Applies the same unified filtering as recruiter search: core skill pre-filter, 40% exact must-have match ratio, engagement model compatibility, and location/availability scoring. CTC budget is a soft indicator (over-budget candidates still match but are flagged).
