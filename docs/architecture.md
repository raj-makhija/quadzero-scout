...
- Applies the same unified filtering as recruiter search: core skill pre-filter, OR semantics for must-have skills: any candidate with at least one exact, fuzzy, or secondary-bucket must-have match passes the inclusion gate; candidates with zero must-have matches are excluded. Candidates rank by their effective must-have score (primary exact + primary fuzzy × 0.85 + secondary × 0.5) normalized by total must-haves, engagement model compatibility, and location/availability scoring. CTC budget is a soft indicator (over-budget candidates still match but are flagged).
