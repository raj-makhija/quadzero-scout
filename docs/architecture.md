...
- **Candidate detail drawer with shortlisting pipeline**: The search results page features a slide-out drawer for each candidate with a guided Search > Screen > Price > Shortlist flow. When a `sourceRequirementId` is available (either from the requirement detail page or after saving a new requirement), the drawer shows: (1) a pipeline progress indicator, (2) screening section with prominent prompts when screening is the blocking step, (3) PricingPanel auto-populated with requirement context (contract duration, payment terms, engagement model) that auto-calculates on mount, and (4) a "Shortlist Candidate" primary CTA that calls `POST /recruiter/shortlist`. For ad-hoc searches without a saved requirement, the drawer shows an info banner prompting the recruiter to save the requirement first.
- **Shortlisting from search results**: Recruiters can shortlist candidates directly from the search page drawer. The handler validates screening freshness client-side (redirecting to screening modal if expired) and handles backend errors (SCREENING_REQUIRED, already shortlisted). After shortlisting, the candidate card shows a green "Shortlisted" badge and the drawer displays a confirmation banner.
